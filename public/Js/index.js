...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (brandsGrid) {
    Object.entries(CARS_DB).forEach(([brand, data]) => {
      ...
      div.addEventListener('click', () => location.href = `cars.ejs?brand=${encodeURIComponent(brand)}`);
  ...
  if (featGrid) {
    ...
    svcs.filter(s => s.popular).slice(0, 8).forEach(svc => {
      ...
      div.innerHTML = `
        <div style="height:120px;margin:-25px -25px 16px -25px;display:flex;align-items:center;justify-content:center;background:${catGrad[svc.cat]||'linear-gradient(135deg,#f5f5f5,#e8e8e8)'};border-radius:var(--radius-md) var(--radius-md) 0 0;overflow:hidden;filter:drop-shadow(0 3px 8px rgba(0,0,0,.12))">
          <img src="${getServiceImageUrl(svc)}" onerror="this.onerror=null;this.src='/services/default-service.jpeg'" style="width:100%;height:100%;object-fit:cover;display:block;" />
        </div>
        ...
        <a href="booking.ejs?service=${svc.id}" class="btn btn-outline btn-sm" style="margin-top:14px; width: 100%">Book Now</a>`;
  ...
  if (tGrid) {
    ...
    const stored = await reviewsAPI.getApproved();
